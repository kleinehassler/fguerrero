...
update usuarios set esptovta=escajero;
update usuarios set escajero='f';
...
CREATE  VIEW vdocuments AS 
 ...
 d.femiauto,  s.scel, s.stelf,  s.saddr, s.saddr2, s.scode, s.ssri, s.sname, s.sruc, s.scedula, ci.descripda AS sciudad, 
 s.spasaporte, j.sname AS vendedor, d.caja, d.efectivo, d.comenanu, d.dirdoc,
 u.sname AS nombreu, monedas.moname AS moneda, d.pflag, d.dflag, d.idws, q.reporte, q.proceso, q.impresora, z.cretri, 
 a.sname as cajero,  t.scode as codter, t.sname as nombret, z.isdevo, z.istransfer, z.documentos,  
 r.sname as recaudador, x1.descripda AS tipdocextraj, s.issocio
   FROM documents d
   ...
   LEFT JOIN detagru x1 on s.tipdocextraj = x1.iddato;
...
CREATE  VIEW varti AS 
 SELECT a.icode AS codigo, a.iname AS nombre, a.artpeso AS peso, a.artcosto AS costo, a.saldocon, a.isiva AS carga_iva, 
   ...
   LEFT JOIN detagru b ON a.ubicacion = b.iddato   
...
CREATE  VIEW vitems AS 
 SELECT a.iditem, a.artgrupo, a.artsgrupo, a.artipo, a.artmarca, a.artpeso, a.artsell, a.artcosto, a.artfob, a.artcosto2, 
 	...
 	a.costopro, a.isserie, a.isembala, a.ubicacion, b.descripda as ubides, a.codbar, s.descripda as subgrupo, a.inactivo,
 	a.iname2
   FROM items a
   LEFT JOIN detagru u ON a.iunidad = u.iddato
   LEFT JOIN monedas m ON a.imoney = m.mocode
...
CREATE VIEW vplancta AS
    SELECT p.nomcuenta AS cuenta, p.plantype, p.plannivel, p.auxcode, p.isdet, p.negativo,
...
CREATE  VIEW vdocui AS 
 SELECT t.code, i.iditem, i.icode, i.iname, i.marca, i.grupo, i.itag, i.genero, i.itipo, i.iname AS auxiliar, i.ubides, 
 	...
 	t.serie2, t.modelo, t.anio, t.cilindraje, t.observ, c.descripda as color, i.isiva as isivai, i.codbar, i.ubicacion
   FROM detadoci t
...
CREATE  VIEW vdocumentg AS 
 SELECT DISTINCT d.code, s.sname AS nombre, d.montototal, v.nomdoc AS documento, d.num, s.scode AS cod_persona, s.ssri AS sri, 
 	...
 	v.dtag, d.pdocode, z.isdevo, z.istransfer, d.chofer, d.fecgra AS fec_grabac, v.iddocu
   FROM documents d
...
CREATE  VIEW vdocusmall AS 
 SELECT DISTINCT d.code, d.userstore, d.usercaja, d.num, d.cliente, d.fecha, d.fto, d.pdocode, d.idsecudoc, d.seller, 
 		...
 		d.flete, d.subtotal, d.subconiva, d.chofer, u.stag, d.caja, d.efectivo, d.comenanu, 
...
CREATE OR REPLACE VIEW vdocuret AS 
 SELECT d.code, d.fecharet AS fecha, d.numsecue1 AS numero, d.montoret, d.numserie1 AS seriedoc, 
	...
	d.numauto2 as numautadq, d.fecha2 as fecdocadq, fcaducidad2 as feccadadq, d.femiauto, 
...
CREATE OR REPLACE VIEW vempleados AS
 SELECT s.scode, 
		...
		e.emarea, g.descripda AS areatrab, profonres, proxiii, proxiv
   FROM empleados e
...
CREATE  VIEW vpreciosv AS 
 SELECT c.descripda AS clientes, i.icode, i.iname, i.artcosto, i.costopro, i.artgrupo, 
 	...
 	p.poruti, p.iddato, g.descripda AS grupo, m.descripda AS marca, fvendes, 
...
CREATE VIEW vsuj AS 
 SELECT s.idsujeto, s.scode, s.sname, s.ssri, s.sruc, s.scedula, s.spasaporte, s.isempleado, s.isproductor, s.isproveedor, 
 		...
 		s.ismayor, s.codprin, s.scontacto, s.saddr2, z.descripda as zona, zona as idzona, s.isccerrado
   FROM sujetos s
   ...
   left join detagru z on s.zona = z.iddato;
...
CREATE VIEW vsujetos AS
    SELECT s.scode AS codigo, s.sname AS nombre, s.ssri AS razon_soc, s.scedula AS cedula, s.sruc AS ruc, s.isccerrado,
    s.spasaporte AS pasaporte, s.isempleado AS es_empleado, s.isproveedor AS es_proveedor, s.iscliente AS es_cliente, 
    s.issocio AS es_socio, c.descripda AS ciudad, p.descripda AS provincia, t.descripda AS tipo_cli, s.saddr AS direccion, 
    s.idsujeto, s.diasplz, s.scredito, s.ismayor, s.codprin, s.scontacto, s.saddr2, z.descripda as zona, zona as idzona
...
CREATE OR REPLACE VIEW votroscre AS 
 SELECT s.scode AS codigo, s.sname AS nombre, s.scedula AS cedula, s.sruc AS ruc, t.descripda AS tipo, s.saddr as direccion, 
	o.diacorte, p.sname as empresa, p.sruc as rucemp, p.saddr as diremp, o.idcredito 
   FROM otroscre o left join sujetos s on (o.idsujeto=s.idsujeto)
   LEFT JOIN detagru t ON o.tipo = t.iddato
   LEFT JOIN sujetos p on (o.idempresa=p.idsujeto);

ALTER TABLE votroscre OWNER TO postgres;
commit;
